...
$launchers = Get-CimInstance Win32_Process | Where-Object {
  ...
  $_.CommandLine -match 'start-hermes\\.(?:js|cjs)' -and
...
function ensureAgentFlowHermesConfig() {
  fs.mkdirSync(hermesHome, { recursive: true });
  const terminalCwd = repoRoot.replace(/\\/g, "/");
  const hermesBaseUrl = process.env.HERMES_BASE_URL || "https://inference-api.nousresearch.com/v1";
  const hermesModel =
    process.env.HERMES_MODEL_DEEP ||
    process.env.HERMES_MODEL_FAST ||
    "Hermes-4-70B";
  const config = `model:
  provider: custom
...
`;
  const existing = fs.existsSync(hermesConfigPath)
    ? fs.readFileSync(hermesConfigPath, "utf8")
    : "";
  if (existing !== config) {
    fs.writeFileSync(hermesConfigPath, config, "utf8");
  }
}
...
function startHermes() {
  if (!ensureSingleLauncher()) {
    // Stay alive so `concurrently` (dev stack) does not see this slot exit on Windows,
    // which often tears down the rest of the backend; Hermes already runs on :8000.
    console.log("[hermes] duplicate launcher skipped — holding slot for dev stack.");
